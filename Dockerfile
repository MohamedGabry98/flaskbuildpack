...
RUN curl -s -L https://www.python.org/ftp/python/3.9.15/Python-3.9.15.tgz > Python-3.9.15.tgz
RUN tar zxvf Python-3.9.15.tgz
RUN rm Python-3.9.15.tgz
WORKDIR /app/.heroku/Python-3.9.15
...
RUN pip install -v firebase==3.0.1
RUN pip install -v firebase-admin==6.0.1
RUN pip install -v gcloud==0.18.3
RUN pip install -v google-api-core==2.10.2
RUN pip install -v google-api-python-client==2.65.0
RUN pip install -v google-auth==2.13.0
RUN pip install -v google-auth-httplib2==0.1.0
RUN pip install -v google-cloud-core==2.3.2
RUN pip install -v google-cloud-firestore==2.7.2
RUN pip install -v google-cloud-storage==2.5.0
RUN pip install -v google-crc32c==1.5.0
RUN pip install -v google-resumable-media==2.4.0
RUN pip install -v googleapis-common-protos==1.56.4
RUN pip install -v python-jwt==4.0.0
